#define AAZZAnalysis_bkg_cxx
#include "/media/daniel/11eb5a4b-2239-4ba5-9d71-965f47b8b84c/AAZZAnalysis_bkg.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void AAZZAnalysis_bkg::Loop()
{
  ...
  */
  ...
  //  bool status = TDirectory::AddDirectoryStatus();
  //TDirectory::AddDirectory(false);

  TString extension, name, name_tree, path;
  extension = ".root";
  //name = "aazz_a0zp10nofflumi100_13tev";
  name ="ppzz_lumi100_pu2x_13tev";
  //name_tree = "ppzz_tree_13tev"; 
  path ="/media/daniel/11eb5a4b-2239-4ba5-9d71-965f47b8b84c/Analise_xicerto_ZZ/";
         

  
  TFile* f = TFile::Open(path+name+extension,"RECREATE");
  f->cd();

  TTree Output_tree("Output_tree","Tree");
  ...
  TH1F* h_ratio   = new TH1F("ratio","ratio",1000,0.,5.);
  ...
  TH1F* h_NZZ4elep_PU= new TH1F("NZZ4elep_PU","NZZ4elep_PU",4,0,4.);
  ...
  // User variables
  Int_t ZZ4u=0, ZZ4e=0, ZZ2u2e=0, NZZ=0, NZZ_sel=0;
 ...
 //=======================================
    ...
    Int_t e1=-1, e2=-1, e3=-1, e4=-1, u1=-1, u2=-1, u3=-1, u4=-1; 
    ...
    Z_on_decay_py[0]=-999.; 
    Z_on_decay_pz[0]=-999.;
    Z_on_decay_energy[0]=-999.;      

    Z_on_decay_px[1]=-999.;   
    Z_on_decay_py[1]=-999.; 
    Z_on_decay_pz[1]=-999.;
    Z_on_decay_energy[1]=-999.;      

    n_decay_off=0;
    Z_off_decay_px[0]=-999.;   
    Z_off_decay_py[0]=-999.; 
    Z_off_decay_pz[0]=-999.;
    Z_off_decay_energy[0]=-999.;      

    Z_off_decay_px[1]=-999.;   
    Z_off_decay_py[1]=-999.; 
    Z_off_decay_pz[1]=-999.;
    Z_off_decay_energy[1]=-999.;      

    isZZee=0; isZZuu=0; isZZue=0; isZZeu=0;
	...
	if( e3 != -1 && e_charge[i]*e_charge[e3] < 0 && i!=e3 ){
      ...
      //------- Choice of Z2 Case 1 -------------------
      if(( u1!=-1 && u2!=-1 ) && ( ( u3!=-1 && u4!=-1 && ( e3==-1 || e4==-1 ) ) || ( u3!=-1 && u4!=-1 && e3!=-1 && e4!=-1 && mu_pt[u3]+mu_pt[u4] >= e_pt[e3]+e_pt[e4] ) ) ){
	TLorentzVector Z_cand; 
	...
	q22.SetPtEtaPhiM(mu_pt[lep2], mu_eta[lep2], mu_phi[lep2], mu_mass[lep2]);
      ...
      //-------------------------------------------------------------------------------------------------------------------
      if( ( u1!=-1 && u2!=-1 ) && ( ( e3!=-1 && e4!=-1 && ( u3==-1 || u4==-1 ) ) || (u3!=-1 && u4!=-1 && e3!=-1 && e4!=-1 && mu_pt[u3]+mu_pt[u4] < e_pt[e3]+e_pt[e4]) ) ){
	TLorentzVector Z_cand; 
	...
	if( e3 != -1 && e_charge[i]*e_charge[e3] < 0 && i!=e1 && i!=e2 && i!=e3 ){
      ...
      if( ( e1!=-1 && e2!=-1 ) && ( ( u3!=-1 && u4!=-1 && ( e3==-1 || e4==-1 ) ) || (u3!=-1 && u4!=-1 && e3!=-1 && e4!=-1 && mu_pt[u3]+mu_pt[u4] >= e_pt[e3]+e_pt[e4]) ) ){ 
	TLorentzVector Z_cand; 
	Z_cand.SetPxPyPzE(mu_px[u3] + mu_px[u4],
			  mu_py[u3] + mu_py[u4],
			  mu_pz[u3] + mu_pz[u4],
	...
	q22.SetPtEtaPhiM(mu_pt[lep2], mu_eta[lep2], mu_phi[lep2], mu_mass[lep2]);
      ...
      //-------------------------------------------------------------------------------------------------------------------    
      if( ( e1!=-1 && e2!=-1 ) && ( ( e3!=-1 && e4!=-1 && ( u3==-1 || u4==-1 ) ) || (u3!=-1 && u4!=-1 && e3!=-1 && e4!=-1 && mu_pt[u3]+mu_pt[u4] < e_pt[e3]+e_pt[e4]) ) ){
	TLorentzVector Z_cand; 
	Z_cand.SetPxPyPzE(e_px[e3] + e_px[e4],
			  e_py[e3] + e_py[e4],
			  e_pz[e3] + e_pz[e4],
      ...
      else if(ZZue != 0 || ZZeu != 0)
	{
      ...
      M_ZZ = ((Z_on+Z_off).M());
      M_ZZ*=gRandom->Gaus(1,0.04);    //4% smearing    
      if(M_ZZ < 195 || M_ZZ >1950) continue;


      mu_lead = mu_leading.Pt();
      mu_sec = mu_second.Pt();
      h_mu_leading_pt->Fill(mu_leading.Pt());			      
      h_mu_second_pt->Fill( mu_second.Pt());     
      ...
      Z_off_py = Z_off.Py();   
      Z_off_pz = Z_off.Pz();   
      Z_off_energy = Z_off.E();   
      
      if(ZZuu != 0){

	n_decay_on=2;
	Z_on_decay_px[0]=mu_leading.Px();   
	Z_on_decay_py[0]=mu_leading.Py(); 
	Z_on_decay_pz[0]=mu_leading.Pz();
	Z_on_decay_energy[0]=mu_leading.E();      
	
	Z_on_decay_px[1]=mu_second.Px();   
	Z_on_decay_py[1]=mu_second.Py(); 
	Z_on_decay_pz[1]=mu_second.Pz();
	Z_on_decay_energy[1]=mu_second.E();


	n_decay_off=2;
	Z_off_decay_px[0]=mu_leading_off.Px();   
	Z_off_decay_py[0]=mu_leading_off.Py();    
	Z_off_decay_pz[0]=mu_leading_off.Pz();   
	Z_off_decay_energy[0]=mu_leading_off.E();         

	Z_off_decay_px[1]=mu_second_off.Px();      
	Z_off_decay_py[1]=mu_second_off.Py();
	Z_off_decay_pz[1]=mu_second_off.Pz();
	Z_off_decay_energy[1]=mu_second_off.E();      
      
	isZZuu=ZZuu;
      } else if(ZZee != 0){

	n_decay_on=2;
	Z_on_decay_px[0]=ele_leading.Px();   
	Z_on_decay_py[0]=ele_leading.Py(); 
	Z_on_decay_pz[0]=ele_leading.Pz();
	Z_on_decay_energy[0]=ele_leading.E();      
	
	Z_on_decay_px[1]=ele_second.Px();   
	Z_on_decay_py[1]=ele_second.Py(); 
	Z_on_decay_pz[1]=ele_second.Pz();
	Z_on_decay_energy[1]=ele_second.E();


	n_decay_off=2;
	Z_off_decay_px[0]=ele_leading_off.Px();   
	Z_off_decay_py[0]=ele_leading_off.Py();    
	Z_off_decay_pz[0]=ele_leading_off.Pz();   
	Z_off_decay_energy[0]=ele_leading_off.E();         

	Z_off_decay_px[1]=ele_second_off.Px();      
	Z_off_decay_py[1]=ele_second_off.Py();
	Z_off_decay_pz[1]=ele_second_off.Pz();
	Z_off_decay_energy[1]=ele_second_off.E();      

	isZZee=ZZee;      
      }else if(ZZeu != 0){
       
	n_decay_on=2;
	Z_on_decay_px[0]=ele_leading_eu.Px();   
	Z_on_decay_py[0]=ele_leading_eu.Py(); 
	Z_on_decay_pz[0]=ele_leading_eu.Pz();
	Z_on_decay_energy[0]=ele_leading_eu.E();      
	
	Z_on_decay_px[1]=ele_second_eu.Px();   
	Z_on_decay_py[1]=ele_second_eu.Py(); 
	Z_on_decay_pz[1]=ele_second_eu.Pz();
	Z_on_decay_energy[1]=ele_second_eu.E();


	n_decay_off=2;
	Z_off_decay_px[0]=mu_leading_off_eu.Px();   
	Z_off_decay_py[0]=mu_leading_off_eu.Py();    
	Z_off_decay_pz[0]=mu_leading_off_eu.Pz();   
	Z_off_decay_energy[0]=mu_leading_off_eu.E();         

	Z_off_decay_px[1]=mu_second_off_eu.Px();      
	Z_off_decay_py[1]=mu_second_off_eu.Py();
	Z_off_decay_pz[1]=mu_second_off_eu.Pz();
	Z_off_decay_energy[1]=mu_second_off_eu.E();      
	...
	n_decay_on=2;
	Z_on_decay_px[0]=mu_leading_ue.Px();   
	Z_on_decay_py[0]=mu_leading_ue.Py(); 
	Z_on_decay_pz[0]=mu_leading_ue.Pz();
	Z_on_decay_energy[0]=mu_leading_ue.E();      
	
	Z_on_decay_px[1]=mu_second_ue.Px();   
	Z_on_decay_py[1]=mu_second_ue.Py(); 
	Z_on_decay_pz[1]=mu_second_ue.Pz();
	Z_on_decay_energy[1]=mu_second_ue.E();

	n_decay_off=2;
	Z_off_decay_px[0]=ele_leading_off_ue.Px();   
	Z_off_decay_py[0]=ele_leading_off_ue.Py();    
	Z_off_decay_pz[0]=ele_leading_off_ue.Pz();   
	Z_off_decay_energy[0]=ele_leading_off_ue.E();         

	Z_off_decay_px[1]=ele_second_off_ue.Px();      
	Z_off_decay_py[1]=ele_second_off_ue.Py();
	Z_off_decay_pz[1]=ele_second_off_ue.Pz();
	Z_off_decay_energy[1]=ele_second_off_ue.E();      
  ...
  cout << "NZZ= " << NZZ << endl;
  cout << "NZZ sel= " << NZZ_sel << endl;

  /*TFile* f_tree = TFile::Open("outtree.root","RECREATE");
  f_tree->cd();
  Output_tree.Print();
  Output_tree.Write();
  f_tree->Close();*/
  ...
  //TDirectory::AddDirectory(status);
